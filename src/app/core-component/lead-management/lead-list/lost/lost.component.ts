...
export class LostComponent {
  ...
  //       if (this.router.url == this.routes.lost) {
  ...
  async openEditModal(
    ...
    this.saveLostData(rawData);
    this.viewLostDetailsDialog = this.dialog.open(templateRef, {
      width: '80%',
    });
  }
  ...
  async copyData(data: any, idx: number) {
    this.saveLostData(data);
    this.helper.copyData(this.lostDetails);
  ...
  updateLostDetails() {
    this.leadManagementService.updateLeadDetails(this.lostDetails).subscribe({
      next: (response: any) => {
        if (response['responseCode'] == '200') {
